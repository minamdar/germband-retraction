% Script to perform PIV from command-line
% Just run this script to see what it does.
% You can adjust the settings in "s" and "p", specify a mask and a region of interest
clc; 
clear all;

% All the directories are saved in cells from which the analysis needs to
% be done. 
% After we are sure that the function works, this should be converted into
% a function
Dirs = cell(5, 1);
...
ExptDir = char(Dirs{dir_i} + SubDir + "/Original");
%% Create list of images inside specified directory
directory = ExptDir; % this is the directory from which the experiments are to be loaded
...
%% PIV analysis loop
% if skp = 1, images in the format 1-2/2-3
% if skp = 2, images in the format 1-2/3-4

skp = 1;
if mod(amount, skp) == 1 %Uneven number of images?
...
counter=0;
%% PIV analysis loop:
% if skp = 1, amount - mod(skp,2) = amount - 1
% if skp = 2, amount - mod(skp,2) = amount, i.e., no change
% here, we choose the beginning and final frames

for i=1:skp:amount - mod(skp, 2)
    counter=counter+1;
    image1=imread(fullfile(directory, filenames{i})); % read images
    image2=imread(fullfile(directory, filenames{i+1}));
    
    % check if image is completely black in a particular image pair
    % actual solution is to not have such images, but temporary solution is
    % to just copy the non-black image into the black image
    if (max(image1(:)) == 0)
        image1 = image2;
    end
    if (max(image2(:)) == 0)
    ...
    [x{counter}, y{counter}, u{counter}, v{counter}, typevector{counter}] = piv_FFTmulti (image1,image2,s{1,2},s{2,2},s{3,2},s{4,2},s{5,2},s{6,2},s{7,2},s{8,2},s{9,2},s{10,2},s{11,2},s{12,2},s{13,2});
    clc
    ...
    %hold on
    %quiver(x{counter},y{counter},u{counter},v{counter},'g','Autoscale', 'off');
    %hold off;
    %axis image;
...
% Settings
umin = -20; % minimum allowed u velocity, adjust to your data
umax = 20; % maximum allowed u velocity, adjust to your data
vmin = -20; % minimum allowed v velocity, adjust to your data
vmax = 20; % maximum allowed v velocity, adjust to your data
...
u_filt=cell((amount - mod(skp, 2))/skp,1);
v_filt=u_filt;
...
for i=1:skp:amount - mod(skp, 2)
    ...
    % Graphical output (disable to improve speed)
    %%{
    imagesc(double(image1)+0*double(image2));colormap('gray');
    hold on
    quiver(x{counter},y{counter},u_filt{counter},v_filt{counter},'r','Autoscale', 'on');
    hold off;
    axis image;
    title(filenames{i},'interpreter','none')
    set(gca,'xtick',[],'ytick',[])
...
%%
%
% saving the filtered data into mat files that could be later loaded for
% additional analysis
%
saveDir = Dirs{dir_i} + SubDir + "/piv";
if ~exist(saveDir, 'dir')
    mkdir(saveDir);
end
SaveFile =  char(saveDir + "/PIVData.mat");
save(SaveFile, 'u_filt', 'v_filt', 'x', 'y');
%%
